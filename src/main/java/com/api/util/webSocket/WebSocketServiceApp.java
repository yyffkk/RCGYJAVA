 ...
 * 所以可以用一个静态set保存起来
 */
@ServerEndpoint(value = "/websocket/app/{nickname}")
@Component
public class WebSocketServiceApp {
    private String nickname;
    private Session session;

    //用来存放每个客户端对应的MyWebSocket对象。
    private static CopyOnWriteArraySet<WebSocketServiceApp> webSocketSet = new CopyOnWriteArraySet<WebSocketServiceApp>();
...
//        this.session.getAsyncRemote().sendText(new Gson().toJson(message));
    ...
    public void broadcast(String message) {
        for (WebSocketServiceApp item : webSocketSet) {
